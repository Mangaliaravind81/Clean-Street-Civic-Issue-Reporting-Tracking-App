...
const AdminSidebar = () => {
      ...
      const rows = data.map((c) => {
        const dateObj = c.created_at
          ...
          : c.createdAt
            ...
            : null;
        const dateStr =
          dateObj && !isNaN(dateObj) ? dateObj.toLocaleDateString() : "N/A";
        const timeStr =
          dateObj && !isNaN(dateObj) ? dateObj.toLocaleTimeString() : "N/A";
  ...
  const SidebarLink = ({ to, icon, label, active }) => (
    <Link
      to={to}
      className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all font-medium ${active ? "bg-blue-600 text-white shadow-lg shadow-blue-100" : "text-gray-400 hover:text-blue-600 hover:bg-blue-50"}`}
    >
    ...
    <aside className="w-64 bg-white border-r border-gray-100 p-6 hidden lg:block shrink-0 sticky top-16 h-[calc(100vh-64px)] overflow-y-auto">
      ...
      <nav className="space-y-2">
        ...
        <SidebarLink
          ...
          active={path === "/manage-complaints"}
        />
        <SidebarLink
          to="/user-management"
          icon={<FaUsers />}
          label="Users"
          active={path === "/user-management"}
        />
            ...
            >
              <div className="p-8 border-b border-gray-50 flex items-center justify-between font-bold text-gray-800 bg-white sticky top-0 z-10">
                <div className="flex items-center gap-2">
                  <FaHistory className="text-blue-500" /> Admin Modification
                  Activity Audit
                </div>
                <button
                ...
                {loadingLogs ? (
                  ...
                  <div className="space-y-3">
                    {recentLogs?.map((log) => (
                      <div
                        key={log._id}
                        className="bg-white border border-gray-100 p-6 rounded-3xl flex items-center justify-between hover:border-blue-200 transition-all shadow-sm"
                      >
