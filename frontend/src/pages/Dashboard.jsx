        ...
        if (userRole === "admin") {
          ...
          if (userRole === "user") {
            myIssues = myIssues.filter(
              (c) => c.user_id && c.user_id._id === userId,
            );
          } else if (userRole === "volunteer") {
            myIssues = myIssues.filter(
              (c) => c.assigned_to && c.assigned_to._id === userId,
            );
          }
          setIssues(myIssues);
        }
        ...
        <header className="mb-12 flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
          <div>
            ...
            <p className="text-gray-400 font-medium">
              Tracking your impact in the community
            </p>
          </div>
          {userRole === "admin" && (
            <Link
              ...
              <table className="w-full text-left border-collapse">
                <thead className="bg-gray-50/50 border-b border-gray-50">
                  <tr>
                    ...
                    <th className="px-6 py-4 text-xs font-bold text-gray-400 uppercase tracking-widest hidden sm:table-cell">
                      Category
                    </th>
                    <th className="px-6 py-4 text-xs font-bold text-gray-400 uppercase tracking-widest">
                      Status
                    </th>
                    <th className="px-6 py-4 text-xs font-bold text-gray-400 uppercase tracking-widest text-right hidden md:table-cell">
                      Reported
                    </th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-50">
                    ...
                    >
                      ...
                      <td className="px-6 py-5">
                        <div className="flex flex-col gap-1">
                          <span
                            className={`text-[10px] font-extrabold px-3 py-1 rounded-full uppercase tracking-tighter shadow-sm border w-fit ${getStatusStyles(issue.status)}`}
                          >
                            {issue.status}
                          </span>
                          {userRole === "volunteer" &&
                            issue.assigned_by &&
                            issue.assigned_by.role === "admin" && (
                  ...
                  {issues.length === 0 && (
                    <tr>
                      ...
                      >
                        No recent activity found. Click "Report Issue" to get
                        started.
                      </td>
                    </tr>
                  )}
...
const ActionCard = ({ to, title, desc, icon, color }) => (
  <Link
    to={to}
    className={`${color} p-6 rounded-3xl shadow-sm flex items-center gap-5 transition-all hover:-translate-y-1 hover:shadow-lg active:scale-95`}
  >
    <div
      className={`p-4 rounded-2xl ${color.includes("bg-white") ? "bg-blue-50 text-blue-600" : "bg-white/20 text-white"}`}
    >
      {icon}
    </div>
    <div>
      ...
      <p
        className={`text-xs ${color.includes("bg-white") ? "text-gray-400" : "text-white/70"} font-medium`}
      >
        {desc}
      </p>
    </div>
