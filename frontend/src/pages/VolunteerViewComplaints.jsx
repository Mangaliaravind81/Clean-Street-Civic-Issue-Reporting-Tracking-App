...
const VolunteerViewComplaints = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        if (token) {
          const vRes = await axios.get("http://localhost:5000/users/volunteers", {
  ...
  const assignComplaint = async (id) => {
    try {
      const token = localStorage.getItem("token");
      await axios.patch(`http://localhost:5000/complaints/${id}`, { volunteer_id: userId }, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleAdminAssign = async (complaintId, volunteerId) => {
    try {
      const token = localStorage.getItem("token");
      await axios.patch(`http://localhost:5000/complaints/${complaintId}`, { volunteer_id: volunteerId }, {
        headers: { Authorization: `Bearer ${token}` }
      });
      await fetchComplaints();
      setNearbyModal(null);
  ...
  const handleEscalate = async (id) => {
    try {
      const token = localStorage.getItem("token");
      const res = await axios.post(`http://localhost:5000/notifications/${id}/escalate`, {}, {
        headers: { Authorization: `Bearer ${token}` }
      });
            ...
            >
                        ...
                        <div className="flex gap-2 w-full">
                          {(() => {
                            ...
                            if (userRole === 'volunteer') {
                              ...
                              return (
                                <button 
                                  onClick={(e) => { e.stopPropagation(); setNearbyModal(item); }}
                                ...
                                >
                                  {item.assigned_to ? 'REASSIGN STAFF' : 'ASSIGN TO STAFF'}
                                </button>
                              );
                            }
                            return null;
              ...
              <div className="p-4 border-t border-slate-50 bg-slate-50/50 flex justify-between items-center px-6">
                ...
                <div className="flex flex-col items-end gap-2">
                  {userRole === "user" && !item.assigned_to && (
                    (() => {
              ...
              <div className="w-full lg:w-1/2 h-[250px] lg:h-[350px] bg-slate-100 relative group/img overflow-hidden border-r border-slate-50">
                <div className="flex h-full overflow-x-auto snap-x snap-mandatory scrollbar-hide">
                  {(selected.photo || (selected.images && selected.images.length > 0)) ? (
                    (Array.isArray(selected.photo) ? selected.photo : (selected.images || [selected.photo])).map((img, idx) => (
                      <div key={idx} className="min-w-full h-full snap-center relative">
                        ...
                        <div className="absolute bottom-4 right-4 bg-black/50 backdrop-blur-md px-3 py-1 rounded-full text-[10px] font-black text-white uppercase tracking-widest">
                          {idx + 1} / {(selected.photo?.length || selected.images?.length || 1)}
                        </div>
                      </div>
                    ))
                  ) : (
                    <div className="w-full h-full flex flex-col items-center justify-center text-slate-300 bg-slate-50 gap-4">
                      ...
                      <span className="text-xs font-black tracking-widest uppercase">No Images</span>
                    </div>
                  )}
                </div>
                { (selected.photo?.length > 1 || selected.images?.length > 1) && (
                  <div className="absolute inset-x-0 bottom-4 flex justify-center gap-2 pointer-events-none">
                     { (Array.isArray(selected.photo) ? selected.photo : selected.images).map((_, i) => (
                      ...
                      <div className="pt-1">
                         ...
                         {selected.assigned_by && (
                           <div className="mt-2 pt-2 border-t border-slate-100">
                             <p className="text-[10px] font-black text-blue-600 uppercase tracking-widest leading-none mb-1">Assigned By</p>
                             <p className="text-[11px] font-bold text-slate-700">{selected.assigned_by.name}</p>
                           </div>
                         )}
                      </div>
                    </div>
                ...
                <div className="space-y-3">
                  {sortedVolunteers.map(v => (
                    <div key={v._id} className="bg-white border border-gray-100 p-5 rounded-3xl flex items-center justify-between hover:border-blue-200 transition-all shadow-sm group">
                       ...
                       <button 
                        onClick={() => handleAdminAssign(nearbyModal._id, v._id)}
