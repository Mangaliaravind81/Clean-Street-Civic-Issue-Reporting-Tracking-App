...
const UserViewComplaints = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      // Fetch Volunteers unconditionally
      try {
      ...
      if (token && userId) {
        try {
          ...
          if (uRes.data.user.location_coords) {
            const [lat, lng] = uRes.data.user.location_coords
              .split(",")
              .map(Number);
  ...
  const handleAdminAssign = async (complaintId, volunteerId) => {
    try {
      const token = localStorage.getItem("token");
      await axios.patch(
  ...
  const rejectComplaint = async (id) => {
    if (!window.confirm("Reject this complaint? It will be hidden for you."))
      return;
    try {
      const token = localStorage.getItem("token");
      await axios.patch(
  ...
  const filteredComplaints = complaints.filter((item) => {
    ...
    if ((userRole === "volunteer" || userRole === "admin") && isRejectedByMe)
      return false;
    ...
    const statusMatch =
      ...
      filterStatus === "Nearby (20km)" ||
      item.status.toLowerCase() === filterStatus.toLowerCase();
    const typeMatch =
      filterType === "All Types" || item.issue_type === filterType;
    const searchMatch =
    ...
    <div className="min-h-screen bg-[#F8FAFC] text-slate-900 pb-20">
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
            <div className="space-y-2">
              ...
              <p className="text-slate-500 max-w-xl text-lg font-medium">
                Real-time visibility into local issues. Track progress, support
                solutions, and connect with your community.
              </p>
            </div>
            ...
            <div className="relative">
              ...
              >
                ...
                {(userRole === "admin" || userRole === "volunteer") && (
                  <option value="Nearby (20km)">Nearby (20km)</option>
                )}
        ...
        {filteredComplaints.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <p className="text-slate-500 mt-2 max-w-sm">
              We couldn't find any issues matching your current filters or
              ...
              >
                <div className="p-5 flex-grow relative">
                  <div className="absolute top-5 right-5">
                    <div
                      className={`px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest border shadow-sm ${getStatusColor(item.status)}`}
                    >
                    ...
                    {item.assigned_to ? (
                      <div className="flex items-center gap-2.5 min-w-0 pl-1">
                        ...
                        <div className="flex flex-col min-w-0">
                          ...
                          <a
                            href={`mailto:${item.assigned_to.email}`}
                            className="text-[9px] font-bold text-slate-400 truncate hover:text-blue-600 transition-colors"
                          >
                          ...
                          <div className="flex gap-2 w-full">
                            {(() => {
                              ...
                              if (staffLocation && item.location_coords) {
                                const [lat, lng] = item.location_coords
                                  .split(",")
                                  .map(Number);
                              ...
                              if (userRole === "volunteer") {
                                ...
                                return (
                                  <button
                                    ...
                                    className={`w-full text-[10px] px-4 py-2.5 rounded-xl font-black tracking-widest transition-all shadow-lg cursor-pointer hover:scale-[1.02] active:scale-95 ${item.assigned_to ? "bg-indigo-50 text-indigo-600 border border-indigo-200 hover:bg-indigo-100 shadow-sm" : "bg-slate-900 text-white hover:bg-blue-600 shadow-blue-200"}`}
                                  >
                                    {item.assigned_to
                                      ? "REASSIGN STAFF"
                                      : "ASSIGN TO STAFF"}
                ...
                <div className="p-4 border-t border-slate-50 bg-slate-50/50 flex justify-between items-center px-6">
                  <div className="flex items-center gap-4">
                    ...
                    >
                      ...
                      <span className="text-xs font-black text-slate-500 group-hover/btn:text-blue-600">
                        {item.upvotes || 0}
                      </span>
                    </button>
                    <button
                    ...
                    >
                      ...
                      <span className="text-xs font-black text-slate-500 group-hover/btn:text-rose-600">
                        {item.downvotes || 0}
                      </span>
                    </button>
                  </div>
      ...
      {selected && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-md flex justify-center items-center z-[10000] p-4 transition-all duration-500">
          <div className="bg-white w-full max-w-4xl max-h-[95vh] overflow-y-auto relative rounded-[2.5rem] shadow-2xl flex flex-col border border-white scrollbar-hide">
            <button
              onClick={() => {
                setSelected(null);
                setIsEditing(false);
            ...
            {/* Modal Top Section: Images & Map Side-by-Side */}
            <div className="flex flex-col lg:flex-row w-full border-b border-slate-100">
              {/* Left: Image Gallery */}
              ...
              <div className="w-full lg:w-1/2 h-[250px] lg:h-[350px] bg-slate-50 relative group/map overflow-hidden">
                ...
                />
                <div className="absolute top-4 left-4 bg-white/90 backdrop-blur px-4 py-2 rounded-2xl text-[10px] font-black uppercase tracking-widest text-slate-800 shadow-xl flex items-center gap-2 border border-slate-100/50">
            ...
            {/* Modal Bottom Section: Details & Comments */}
            <div className="p-10 flex flex-col bg-slate-50/10">
              {!isEditing ? (
                <div className="mb-10 animate-in fade-in slide-in-from-right-4 duration-500">
                  <div className="flex items-center gap-3 mb-6">
                    <div
                      className={`px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest border shadow-sm ${getStatusColor(selected.status)}`}
                    >
                  ...
                  <div className="flex flex-wrap gap-2 mb-8">
                    <span className="bg-white border border-slate-200 px-4 py-2 rounded-2xl text-xs font-black text-slate-600 uppercase tracking-wider shadow-sm flex items-center gap-2">
                      <span className="text-lg">
                        {getIssueIcon(selected.issue_type)}
                      </span>
                      {selected.issue_type}
                    </span>
                    <span
                      className={`px-4 py-2 rounded-2xl text-xs font-black uppercase tracking-wider border shadow-sm flex items-center gap-2 ${selected.priority === "High" ? "bg-rose-50 border-rose-100 text-rose-600" : "bg-white border-slate-200 text-slate-600"}`}
                    >
                      <div
                        className={`w-2 h-2 rounded-full ${selected.priority === "High" ? "bg-rose-500 animate-pulse" : "bg-slate-400"}`}
                      ></div>
                      ...
                      <div className="pt-1">
                        ...
                        {userRole === "volunteer" || userRole === "admin" ? (
                          <>
                            <p className="text-slate-700 font-bold leading-tight">
                              Reporter:{" "}
                              {selected.user_id?.name || "Verified Citizen"}
                            </p>
                            <a
                          ...
                          <>
                            <p className="text-slate-700 font-bold leading-tight">
                              Handled By: {selected.assigned_to.name}
                            </p>
                            <a
                          ...
                          <>
                            <p className="text-slate-700 font-bold leading-tight cursor-pointer">
                              Reporter:{" "}
                              {selected.user_id?.name || "Verified Citizen"}
                            </p>
                            <p className="text-[11px] text-slate-400 font-medium mt-1 italic italic">
                          ...
                          <div className="pt-1">
                            ...
                            <div className="flex flex-wrap gap-1 mt-1">
                              {selected.rejected_by.map((u) => (
                                <span
                                  key={u._id}
                                  className="text-[10px] font-black bg-rose-50 text-rose-600 px-2 py-0.5 rounded border border-rose-100 uppercase"
                                >
                                  {u.name}
                                </span>
                              ))}
                ...
                <div className="mb-10 space-y-6 animate-in fade-in slide-in-from-right-4 duration-500">
                  <div className="flex items-center justify-between pb-4 border-b border-slate-100">
                    <h2 className="text-2xl font-black text-slate-900 italic">
                      Adjust{" "}
                      <span className="text-blue-600 text-not-italic">
                        Report Details
                      </span>
                    </h2>
                  ...
                  <div className="space-y-4">
                    <div className="group/field">
                      <label className="block text-[10px] font-black text-slate-400 mb-2 uppercase tracking-widest group-focus-within/field:text-blue-600 transition-colors">
                        Issue Title
                      </label>
                      <input
              ...
              <div className="flex-grow flex flex-col min-h-0">
                <div className="flex items-center justify-between mb-6">
                  <h4 className="font-black text-slate-800 flex items-center gap-3 text-xs uppercase tracking-[0.2em]">
                    <FaRegComment className="text-blue-500" /> Community
                    Discussion
                  </h4>
                ...
                <div className="space-y-4 mb-4 overflow-y-auto pr-2 custom-scrollbar flex-grow min-h-[150px]">
                  {selected.comments && selected.comments.length > 0 ? (
                    selected.comments.map((c) => (
                      <div
                        key={c._id}
                        className="bg-white p-5 rounded-[2rem] border border-slate-100 shadow-sm relative group/msg"
                      >
                        <div className="flex justify-between items-center mb-2">
                          <span className="font-black text-[10px] text-blue-600 uppercase tracking-widest">
                            {c.user_id?.name || "Community Member"}
                          </span>
                          <div className="flex items-center gap-3">
                            ...
                            {(userId === c.user_id?._id ||
                              ...
                              <button
                                onClick={() =>
                                  deleteComment(selected._id, c._id)
                                }
                ...
                <div className="mt-auto relative">
                  <input
                    ...
                    onKeyPress={(e) =>
                      e.key === "Enter" && addComment(selected._id)
                    }
                  />
                  <button
      ...
      {nearbyModal &&
        (() => {
          let cLat = 0,
            cLng = 0;
          if (nearbyModal.location_coords) {
            [cLat, cLng] = nearbyModal.location_coords.split(",").map(Number);
          }

          const sortedVolunteers = [...volunteers]
            .map((v) => {
              let distance = Infinity;
              if (v.location_coords && cLat !== 0) {
              ...
              <div className="bg-white w-full max-w-lg rounded-[2.5rem] shadow-2xl relative max-h-[85vh] flex flex-col overflow-hidden animate-in zoom-in-95 duration-200">
                ...
                <div className="overflow-y-auto p-4 scrollbar-hide bg-slate-50/50 flex-grow">
                  <div className="space-y-3">
                    {sortedVolunteers.map((v) => (
                      <div
                        key={v._id}
                        className="bg-white border border-gray-100 p-5 rounded-3xl flex items-center justify-between hover:border-blue-200 transition-all shadow-sm group"
                      >
                        <div className="flex items-center gap-4">
                        ...
                        <button
                          onClick={() =>
                            handleAdminAssign(nearbyModal._id, v._id)
                          }
                ...
                <div className="p-5 bg-white border-t border-gray-50 text-center">
                  <p className="text-xs text-gray-400 font-medium">
                    Displaying volunteers sorted by proximity to the issue.
                  </p>
                </div>
              </div>
            </div>
